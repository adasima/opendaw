...
pub fn draw_arranger(ui: &mut egui::Ui, app: &mut crate::app::OpenDawApp) {
    ...
    while t <= max_tick {
        ...
        grid_painter.line_segment(
            [
                Pos2::new(x, track_area_rect.min.y),
                Pos2::new(x, track_area_rect.max.y),
            ],
            stroke,
        );
    ...
    for i in 0..num_mock_tracks {
        ...
        grid_painter.line_segment(
            [
                Pos2::new(track_area_rect.min.x, y + arranger.track_height),
                Pos2::new(track_area_rect.max.x, y + arranger.track_height),
            ],
        ...
        if i == 0 {
            ...
            let clip_rect = Rect::from_min_max(
                Pos2::new(clip_x1, y + 4.0),
                Pos2::new(clip_x2, y + arranger.track_height - 4.0),
            );
            ...
            grid_painter.rect_stroke(
                clip_rect,
                4.0,
                Stroke::new(1.0, Color32::from_rgb(100, 180, 250)),
    ...
    let playhead_x = rect.min.x
        + (state.playhead_pos * arranger.ticks_per_beat as f32) * arranger.pixels_per_tick
        - arranger.pan.x;
