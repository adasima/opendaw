...
pub fn get_midi_devices() -> Vec<String> {
    ...
    vec![
        "Launchkey Mini".to_string(),
        "Scarlett 2i2 USB".to_string(),
        "Virtual MIDI Bus".to_string(),
    ]
}

/// トラックに対するMIDIデバイスとチャンネルのルーティングを設定する
#[tauri::command]
pub fn set_track_midi_routing(
    track_id: u32,
    device: String,
    channel: u8,
    ...
    info!(
        "MIDI Route: Set track {} to device '{}' channel {}",
        track_id, device, channel
    );
...
/// トラックの出力ルーティングを設定する
#[tauri::command]
pub fn set_track_output_routing(
    track_id: usize,
    target: Option<usize>,
    state: State<'_, AppState>,
) -> Result<(), String> {
...
pub fn update_automation_point(
    ...
    if let Some(track) = project_state.tracks.iter_mut().find(|t| t.id == track_id) {
        ...
        if let Some(existing) = auto_track
            .points
            .iter_mut()
            .find(|p| (p.time - time).abs() < 0.001)
        {
...
/// オートメーションの表示状態を設定する
#[tauri::command]
pub fn set_automation_visibility(
    track_id: usize,
    visible: bool,
...
pub fn add_track_send(
    ...
    if let Some(track) = project_state.tracks.iter_mut().find(|t| t.id == track_id) {
        ...
        if !track
            .sends
            .iter()
            .any(|s| s.target_track_id == target_track_id)
        {
...
pub fn set_track_send_amount(
    ...
    if let Some(track) = project_state.tracks.iter_mut().find(|t| t.id == track_id) {
        if let Some(send) = track
            .sends
            .iter_mut()
            .find(|s| s.target_track_id == target_track_id)
        {
            send.amount = amount;
            state
                .engine
