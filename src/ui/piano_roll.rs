...
impl PianoRoll {
    ...
    pub fn show(&mut self, ui: &mut egui::Ui) -> egui::Response {
        ...
        // Cursor icon for resizing
        if self.resizing_note.is_some() {
        ...
        if response.dragged_by(PointerButton::Primary) {
            if let Some((id, offset)) = self.dragging_note {
                ...
                if let Some(pos) = pointer_pos {
                    let grid_pos = pos - grid_rect.min + self.pan;
                    let end_tick = (grid_pos.x / self.pixels_per_tick).max(0.0) as u32;
                    let snap_step = self.ticks_per_beat / 4;
                    // snap to 16th
                    let end_tick = ((end_tick + snap_step / 2) / snap_step) * snap_step;

                    if let Some(note) = self.notes.iter_mut().find(|n| n.id == id) {
                        if end_tick > note.start_tick {
                            note.duration = end_tick - note.start_tick;
                        } else {
