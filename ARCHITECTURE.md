...
│   └── session.rs       # セッションビューのClip/Scene定義など
│
├── plugin/              # (Sanctuary) プラグインホスティング基盤 (人間が管理)
│   └── host.rs          # VST3 / CLAP プラグインの安全なロードとラッパー
│
...
### 4. ハイブリッド開発（聖域ルール）
- `src/core/` や `src/plugin/` などの高度な基盤ディレクトリは「聖域（Sanctuary）」とし、人間（ユーザー）が直接管理・実装を行います。
- AIエージェントはこれらの聖域ディレクトリ内のファイルを直接変更してはいけません（APIを呼び出して利用するのは許可されます）。
