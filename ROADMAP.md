...
- [ ] nova: [2-1] opendaw-wasm/src/engine/mixer.rs からルーティング処理を切り分け、新しく `opendaw-wasm/src/engine/routing.rs` を作成する (対象: opendaw-wasm/src/engine/mixer.rs, opendaw-wasm/src/engine/routing.rs, opendaw-wasm/src/engine/mod.rs)
- [ ] nova: [2-2] opendaw-wasm/src/engine/stream.rs からバス処理を切り分け、新しく `opendaw-wasm/src/engine/bus.rs` を作成する (対象: opendaw-wasm/src/engine/stream.rs, opendaw-wasm/src/engine/bus.rs, opendaw-wasm/src/engine/mod.rs)
- [x] nova: [3] opendaw-wasm/src/ui/piano_roll.rs を描画コンポーネント（グリッド、ノート等）ごとに整理・分割する (対象: opendaw-wasm/src/ui/piano_roll.rs)
- [ ] nova: [4-1] opendaw-wasm/src/state/track.rs を機能ドメインごとのモジュールに整理・分割し、新たに `opendaw-wasm/src/state/track_clip.rs` や `opendaw-wasm/src/state/track_plugin.rs` を作成する (対象: opendaw-wasm/src/state/track.rs, opendaw-wasm/src/state/track_clip.rs, opendaw-wasm/src/state/track_plugin.rs)
- [ ] nova: [4-2] opendaw-wasm/src/state/mod.rs を機能ドメインごとのモジュールに整理・分割し、再エクスポートを整理する (対象: opendaw-wasm/src/state/mod.rs)
- [ ] nova: [5-1] opendaw-wasm/src/engine/synth.rs を波形生成モジュールに分割し、新しく `opendaw-wasm/src/engine/oscillator.rs` を作成する (対象: opendaw-wasm/src/engine/synth.rs, opendaw-wasm/src/engine/oscillator.rs, opendaw-wasm/src/engine/mod.rs)
- [ ] nova: [5-2] opendaw-wasm/src/engine/synth.rs をボイス管理モジュールに分割し、新しく `opendaw-wasm/src/engine/voice.rs` を作成する (対象: opendaw-wasm/src/engine/synth.rs, opendaw-wasm/src/engine/voice.rs, opendaw-wasm/src/engine/mod.rs)
