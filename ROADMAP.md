...
- [ ] [4] @A src/ui/piano_roll.rs を更新し、ノートの端のドラッグによる長さ（duration）変更を可能にするUIインタラクションを実装する (対象: src/ui/piano_roll.rs)
- [ ] [5] @B src/state/track.rs を更新し、Track構造体に `midi_clips: Vec<crate::state::clip::MidiClip>` フィールドを追加し、初期化とシリアライズを構成する (対象: src/state/track.rs)
- [ ] [6] @B src/app.rs を更新し、ピアノロールUIでの変更（グローバルな active_sequence）を選択中トラックの `MidiClip` に同期・保存するロジックを実装する (対象: src/app.rs)
- [ ] [7] @B src/app.rs を更新し、`process_active_notes` メソッドを改修して各トラックの `midi_clips` から独立して発音を計算し、重複発音を防止するロジックを実装する (対象: src/app.rs)
