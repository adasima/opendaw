...
- 300行を超えるファイル → 「`app.rs` が310行。分割を推奨」
- 設計上の懸念 → 「`engine/` 内で `Arc<Mutex<T>>` が使われている。リアルタイム制約に違反」
...
### 発見事項
- ⚠️ `src/app.rs` (310行): モジュール分割を推奨。UI描画ロジックと状態管理が混在。
- ⚠️ `src/engine/stream.rs`: オーディオコールバック内で `format!()` を使用。
```
