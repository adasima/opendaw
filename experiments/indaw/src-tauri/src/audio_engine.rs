    ...
    pub fn start(&mut self, device_name: Option<String>) -> Result<()> {
        ...
        let device = if let Some(name) = device_name {
            ...
            host.default_output_device()
                .context("Failed to find default output device")?
        };
        ...
        let stream = match config.sample_format() {
            cpal::SampleFormat::F32 => run::<f32>(
                &device,
                &config.into(),
                self.cmd_receiver_internal.clone(),
                self.meter_sender_internal.clone(),
                self.playhead_sender_internal.clone(),
        ...
        move |data: &mut [T], _: &cpal::OutputCallbackInfo| {
            ...
            if let Ok(tracks_guard) = shared_state.tracks.try_read() {
                let any_solo = tracks_guard
                    .iter()
                    .any(|t| t.soloed.load(Ordering::Relaxed));
                ...
                for track in tracks_guard.iter() {
                        ...
                        match &**content {
                            crate::state::TrackContent::Midi(seq) => {
                                while runtime.next_note_idx < seq.notes.len() {
                                    ...
                                    if note.start_time >= buffer_end_pos {
                                        break;
                                    }
                                    let release_end =
                                        note.start_time + note.duration + synth_params.release;
                                    ...
                                    runtime.active_voices.push(Voice {
                                        note: note.note,
                                        frequency: crate::synth::note_to_frequency(note.note),
                                        velocity: note.velocity as f32
                                            / constants::MIDI_VELOCITY_MAX,
                    ...
                    runtime
                        .active_voices
                        .retain(|v| v.release_end_time > current_pos_sec);
                }
                ...
                struct ActiveRenderer<'a> {
                    id: usize,
                    content:
                        std::sync::RwLockReadGuard<'a, std::sync::Arc<crate::state::TrackContent>>,
                    voices: &'a [Voice],
                    vol: f32,
                    pan_l: f32,
                    pan_r: f32,
                ...
                for frame in data.chunks_mut(channels) {
                        ...
                        for renderer in &active_renderers {
                            ...
                            if let crate::state::TrackContent::Midi(_) = &**renderer.content {
                                for voice in renderer.voices {
                                    if current_pos_sec >= voice.start_time
                                        && current_pos_sec < voice.release_end_time
                                    {
                                        let t = current_pos_sec - voice.start_time;
                                        let duration = voice.end_time - voice.start_time;
                                        let envelope = crate::synth::calculate_envelope(
                                            t,
                                            duration,
                                            &synth_params,
                                        );

                                        let phase = current_pos_sec as f32
                                            * voice.frequency
                                            * 2.0
                                            * std::f32::consts::PI;
                                        let osc_val = crate::synth::calculate_oscillator(
                                            phase,
                                            synth_params.oscillator_type,
                                        );
                                        track_sample += osc_val
                                            * constants::SYNTH_OUTPUT_GAIN
                                            * envelope as f32
                                            * voice.velocity;
                                    }
                                }
                            } else if let crate::state::TrackContent::Audio(clip) =
                                &**renderer.content
                            {
                                let sample_idx =
                                    (current_pos_sec * clip.sample_rate as f64).floor() as usize;
                                if dim_check_ok(sample_idx, clip) {
                                    if clip.channels == 2 {
                                        let idx = sample_idx * 2;
                                        if idx + 1 < clip.samples.len() {
                                            track_sample +=
                                                (clip.samples[idx] + clip.samples[idx + 1]) * 0.5;
                                        }
                            ...
                            if track_l.abs() > peaks[0] {
                                peaks[0] = track_l.abs();
                            }
                            if track_r.abs() > peaks[1] {
                                peaks[1] = track_r.abs();
                            }
                    ...
                    // Apply Master Volume
                    let master_vol =
                        f64::from_bits(shared_state.master_volume.load(Ordering::Relaxed)) as f32;
                    mix_left *= master_vol;
                    mix_right *= master_vol;

                    // Metering Master accumulation
                    if mix_left.abs() > acc_master_peak[0] {
                        acc_master_peak[0] = mix_left.abs();
                    }
                    if mix_right.abs() > acc_master_peak[1] {
                        acc_master_peak[1] = mix_right.abs();
                    }

                    if channels >= 2 {
                        if let Some(s) = frame.get_mut(0) {
                            *s = cpal::Sample::from_sample(mix_left);
                        }
                        if let Some(s) = frame.get_mut(1) {
                            *s = cpal::Sample::from_sample(mix_right);
                        }
            ...
            if playing {
                shared_state
                    .playhead_position
                    .store(current_pos_sec.to_bits(), Ordering::Relaxed);
