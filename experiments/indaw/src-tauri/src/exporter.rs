                     ...
                     TrackContent::Midi(seq) => {
                         for note in &seq.notes {
                              ...
                              if current_time >= note.start_time && current_time < release_end {
                                  ...
                                  let envelope = crate::synth::calculate_envelope(t, note.duration, &synth_params);

                                  let freq = crate::synth::note_to_frequency(note.note);
                                  let phase = current_time as f32 * freq * 2.0 * std::f32::consts::PI;
                                  let osc_val = crate::synth::calculate_oscillator(phase, synth_params.oscillator_type);

                                  track_sample += osc_val * crate::constants::SYNTH_OUTPUT_GAIN * envelope as f32 * (note.velocity as f32 / 127.0);
                              }
                         }
                     },
                     TrackContent::Audio(clip) => {
